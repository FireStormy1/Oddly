...
function handleLeave(io: Server, socket: Socket) {
  ...
  const player = room.players[socket.id];
  if (player) {
    logger.info({ roomCode, playerName: player.name }, "Player left room");
  }
  ...
  if (connected.length === 0) {
    ...
    logger.info({ roomCode }, "Room deleted (empty after leave)");
    return;
  }
  ...
  if (room.hostId === socket.id && connected.length > 0) {
    ...
    logger.info({ roomCode, newHost: connected[0] }, "Host transferred after leave");
  }
